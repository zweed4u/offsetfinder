...
for device in ota_json.keys():
	...
	if 'iPhone' in device:  # only iphones
		...
		kernelcache_location += [string for string in os.listdir(kernelcache_location) if 'kernelcache.release.' in string][0]
		result = subprocess.run(['joker', '-dec', f'{kernelcache_location}'], stdout=subprocess.PIPE)
		print(str(result.stdout.decode('utf8')))
		jtool_symbol = subprocess.run(['jtool', '-S', '/tmp/kernel'], stdout=subprocess.PIPE)
